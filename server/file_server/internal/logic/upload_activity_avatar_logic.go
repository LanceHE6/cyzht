...
type UploadActivityAvatarLogic struct {
	ctx    context.Context
	svcCtx *svc.ServiceContext
	logx.Logger
}

func NewUploadActivityAvatarLogic(ctx context.Context, svcCtx *svc.ServiceContext) *UploadActivityAvatarLogic {
	return &UploadActivityAvatarLogic{
		ctx:    ctx,
		svcCtx: svcCtx,
...
func (l *UploadActivityAvatarLogic) UploadActivityAvatar(in *file_server.UploadAvatarRequest) (*file_server.UploadAvatarResponse, error) {
	// 生成唯一的文件ID，这里简单使用时间戳+自增数字（实际可使用更严谨的UUID等方式）
	filename := strconv.FormatInt(time.Now().UnixNano(), 10)
	avatarPath := l.svcCtx.Config.StoragePath + "/activity_avatar/" + filename + in.FileType
	url := "/activity_avatar/" + filename + in.FileType
	// 保存文件到本地磁盘
