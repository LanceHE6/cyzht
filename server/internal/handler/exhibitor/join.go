...
func (e *exhibitorHandler) JoinExhibitor(ctx *gin.Context) {
	...
	activity, err := e.ExhibitorRepo.SelectByID(eid)
	if activity == nil || err != nil {
		ctx.JSON(http.StatusOK, response.FailedResponse(1, "参展商不存在"))
		return
	}
