...
func (s userHandler) RegisterAndLoginSendCode(ctx *gin.Context) {
	...
	if u != nil {
		...
		if err != nil {
			ctx.JSON(http.StatusInternalServerError, response.ErrorResponse(10, "发送验证码失败", err))
			return
		}
		// 验证码写入redis 5分钟
		...
		if err != nil {
			ctx.JSON(http.StatusInternalServerError, response.ErrorResponse(11, "发送验证码失败", err))
			return
		}
		// 验证码写入redis 5分钟
		s.VerifyCodeRepo.SetVerifyCode(data.Account, code)
...
func (s userHandler) RegisterAndLoginVerifyCode(ctx *gin.Context) {
	type registerRequest struct {
		Account string `json:"account" form:"account" binding:"required"`
...
func (s userHandler) Login(ctx *gin.Context) {
	// loginRequest
	//
