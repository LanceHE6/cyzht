...
func (a *activityHandler) PublishNotice(ctx *gin.Context) {
	...
	data := bindparams.BindPostParams[publishNoticeRequest](ctx)
	if data == nil {
		return
	}
	claims, _ := jwt.GetClaimsByContext(ctx)
	if _, err := a.ActivityNoticeRepo.Insert(&model.ActivityNoticeModel{
		ActivityID: aid,
		Title:      data.Title,
